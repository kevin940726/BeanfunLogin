    ...
    public partial class BeanfunClient : WebClient
    {
        ...
        public string GetOTP(int loginMethod, AccountList acc)
        {
            try
            {
                string response;
                if (loginMethod == 5)
                    response = this.DownloadString("https://tw.beanfun.com/beanfun_block/game_zone/game_start_step2.aspx?service_code=610074&service_region=T9&sotp=" + acc.sotp + "&dt=" + GetCurrentTime(2), Encoding.UTF8);
                else
